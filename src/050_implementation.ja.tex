...
% 保存ボタンを押したタイミングで、JSを実行し、データを取得


% ↑で使用するHTMLのAPIを書く
表\ref{tb:}に、JavaScriptで取得するデータと取得するために用いるAPIをまとめる。
% 動画再生位置は、一番上の動画。ない場合は0。
...
% ↑で取得したデータは、iOSアプリケーション内のShareViewControllerというコントローラーに渡される。

% ShareViewControllerは上記のデータにメタデータを付け加え、サーバに保存する。
